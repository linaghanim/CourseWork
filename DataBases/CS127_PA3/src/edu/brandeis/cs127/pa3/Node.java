...
/**
 * Superclass of Leaf Nodes (LeafNode) and Internal Nodes (InternalNode) of
 * B+-Trees.
 * 
 * @author cs127b
 */
public abstract class Node {
	 ...
	 * 
	 * @return a node which is next to this node, or null if this node is the last
	 *         one of one level
	 */
	public Node getNext() {
		return next;
	 ...
	 * 
	 * @return a node which is right before this node, or null if this node is the
	 *         first one of a level
	 */
	public Node getPrev() {
		return prev;
	}
	 ...
	 * 
	 * @return a reference which refers to the parend node, or null if this node is
	 *         the root
	 */
	...
	/**
	 * Count the key number of the level this node is in. (for print purpose)
	 */
	...
	/**
	 * Checks whether two Node objects are siblings. Two instances of Node are
	 * siblings if they are children of the same node.
	 * 
	 * @param other
	...
	/**
	 * Find index i s.t. keys [i] == val, or if val were to be inserted, keys [i] is
	 * where it should be placed. More formally, returns i s.t. keys [i-1] < val <=
	 * keys [i], or lastindex if val >= keys [lastindex]
	 * 
	 * @param val
	 *            the value to find
	...
	public int findKeyIndex(int val) {
		...
		while (i <= this.lastindex && keys[i] <= val) {
			i++;
		}
	...
	/**
	 * Find index i s.t. ptrs [i] is the pointer that should be followed in the
	 * search for val. More formally, returns i s.t. keys [i] <= val < keys [i + 1]
	 * or 0, if val < keys [1]. Note, cannot compute this result by incrementing or
	 * decrementing result of findKeyIndex
	 * 
	 * @param val
	...
	public int findPtrIndex(int val) {
		...
		while (index <= this.lastindex && keys[index] < val) {
			index++;
		}
	...
	/**
	 * Shifts pointers and keys beginning at ptrs [i] and keys [i + 1] as far left
	 * in the node as they can go, overwriting the current pointers and keys stored
	 * there.
	 * 
	 * @param i
	 *            the position it shifts from.
	 */
	...
	int getDepth() {
		...
		while ((p = p.ptrs[0]) != null)
			i++;
		return i;
	}

	/**
	 * Delete the key and pointer at index i. Combine or redistribute nodes if
	 * necessary to preserve order of node.
	 * 
	 * @param i
	 *            the index of key and pointer to delete
	 */
	public void delete(int i) {
		...
		// only when underfull
		if (this.lastindex < Math.ceil((this.degree - 1) / 2.0)) {
			if (this.siblings(this.getNext()) && this.combinable(this.getNext())) {
				int parent = this.getNext().keys[1];
				this.combine();
				if (this.parentref != null) {

					this.parentref.getNode().delete(this.parentref.getNode().findKeyIndex(parent));

				}
			} else if (this.siblings(this.getPrev()) && this.combinable(this.getPrev())) {
				int parent = this.keys[1];
			...
			} else if (this.siblings(this.getNext())) {
				this.redistribute();
				this.getParent().getNode().keys[this.getParent().getNode().findKeyIndex(d)] = this.getNext().keys[1];
			} else if (this.siblings(this.getPrev())) {
				this.getPrev().redistribute();
				this.getParent().getNode().keys[this.getParent().getNode().findKeyIndex(d)] = this.keys[1];
			}
			...
			if (i == 1) {
				...
				this.parentref.getNode().keys[this.parentref.getNode().findKeyIndex(d)] = d;
			}
		}
	...
	/**
	 * The minimum number of keys this node should have, which differs from
	 * {@link LeafNode} and {@link InternalNode}.
	 * 
	...
	/**
	 * Combines the contents of this node and its next sibling (next) into a single
	 * node
	 */
	public abstract void combine();

	/**
	 * Redistributes keys and pointers in this node and its next slibing so that
	 ...
	 * @param i
	 *            the index where the key and pointer should be
	 */
	public abstract void insertSimple(int val, Node ptr, int i);

	/**
	 * Deletes keys [i] and ptrs [i] from this node, without performing any
	 * combination or redistribution afterwards. It is done by shifting all keys and
	 * pointers from index i+1 on one position to the left.
	 * 
	 * @param i
	 *            the index of key and pointer to delete
	 */
	...
	/**
	 * Equivalent to {@link #findKeyIndex} in {@link LeafNode} but calls itself
	 * recursively in {@link InternalNode}.
	 * 
	...
	/**
	 * The difference between {@link LeafNode} and {@link InternalNode} is whether a
	 * split creates a {@link LeafNode} or {@link InternalNode}, and whether the
	 * split results in a duplicate key being inserted into parent or not. (For
	 * {@link LeafNode}, yes. For {@link InternalNode}, no.)
	 * 
	 * @param val
	 *            the value to insert
	 * @param ptr
